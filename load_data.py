...
with open('dataset_without_modules.pickle', 'rb') as h:
    data = pickle.load(h)
...
ax[1].set_ylabel('$\\phi(Z)$')
for measurement in p1_h:
    frequency = np.array(measurement['frequency'])  # Convert back to numpy array
    z = np.array(measurement['z'])  # Convert back to numpy array
    ax[0].plot(frequency, np.abs(z))
    ax[1].plot(frequency, np.angle(z))
plt.show()
